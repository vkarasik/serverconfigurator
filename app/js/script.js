$(function () {
  ...
  $('#add-cmpt').click(function () {
    ...
    $('#confurator-table tbody').append(
      '<tr id="' +
        newAdditionalId +
        '" class="addition-component"><td class="component"><a href="#" class="add-cmpt" data-toggle="modal" data-target=".bs-example-modal-lg" data-component="' +
        additionalDataCmpt +
        '">' +
        additionalComponent.text() +
        '</a></td><td class="name"></td><td class="shortname"></td><td class="price"></td><td class="price-hidden"></td><td class="avail"></td><td class="number"></td><td class="del-cmpt"><button type="button" class="btn btn-danger btn-xs" title="Удалить компонент"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button></td></tr>'
    );
    ...
    $.getJSON(jsonSource, { cache: false }, function (data) {
      ...
      if ($('.checkbox input')['0'].checked) {
        ...
        if (curComponent == 'platform') {
          ...
          for (i = 0; i < curComponentArr.length; i++) {
            ...
            if (curLink !== '') {
              curLink = '<br><a href="' + curLink + '" target="_blank" rel="nofollow noopener" rel=noreferrer>Подробное описание...</a>';
            }
            ...
            if (curRelevant.indexOf(curPlatform) + 1) {
              ...
              $(subCat).after(
                '<tr class="component-item" data-relevant="' +
                  curRelevant +
                  '"><td class="cmpt-name">' +
                  curComponentArr[i].name +
                  curLink +
                  '</td><td class="cmpt-shortname" style="display:none">' +
                  curComponentArr[i].shortname +
                  '</td><td class="cmpt-price">' +
                  curComponentArr[i].price +
                  '</td><td class="cmpt-avail">' +
                  curComponentArr[i].availability +
                  '</td><td class="choose"><button class="btn btn-primary choose-cmpt" data-dismiss="modal">Выбрать</button></td></tr>'
              );
            }
          }
      ...
      // Показывать все компоненты
        ...
        for (i = 0; i < curComponentArr.length; i++) {
          ...
          // строка с текущей подкатегорией
          var subCat;
          $('.subcategory td').each(function (index, el) {
            if (el.innerHTML === curSubCat) {
              subCat = $(this).parent();
            }
          });
